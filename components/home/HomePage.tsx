...
import { ArrowRight, MapPin, Home, Building2, Waves, Sprout, TrendingUp, ChevronLeft, ChevronRight, ChevronDown, DollarSign, Heart, FileText, CheckCircle2, Check, Phone, MessageCircle } from "lucide-react";
...
const PLOT_FINDER_POINTS = [
  ...
  "Water and power on selected sites",
  "Projects from Mariakani to Malindi",
  "A deposit, then monthly installments",
];
...
function CoastalPlotFinder() {
  ...
  return (
    <section id="find-a-kilifi-plot" aria-labelledby="find-a-kilifi-plot-heading" className="bg-white pb-16 pt-2 md:pb-20">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="mt-1 font-montserrat text-base font-semibold text-primary-700 md:text-lg">
            Open listings begin at KES 395,000
          </p>
          ...
          <form
            ...
            className="mx-auto mt-8 flex w-full max-w-xl items-center rounded-full border border-dark-200 bg-white p-1.5 shadow-md"
          >
            <div className="relative min-w-0 flex-1">
              ...
              <ChevronDown
                size={18}
                className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-dark-400"
              />
            </div>
            <button
              type="submit"
              className="shrink-0 rounded-full bg-primary-600 px-4 py-3 font-montserrat text-sm font-semibold text-white transition hover:bg-primary-700 sm:px-6 sm:text-base"
            >
              View plots
            </button>
          </form>
          ...
          <ul className="mt-5 flex flex-wrap items-center justify-center gap-x-5 gap-y-2">
            {PLOT_FINDER_POINTS.map((point) => (
              <li key={point} className="flex items-center gap-1.5 font-montserrat text-sm text-dark-600">
                <Check size={16} className="shrink-0 text-primary-600" strokeWidth={2.5} />
                {point}
              </li>
            ))}
          </ul>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full border-2 border-primary-600 px-6 py-3 font-montserrat font-semibold text-primary-700 transition hover:bg-primary-50"
            >
              <Phone size={18} />
              Talk to an Expert
            </a>
            <a
              href={whatsAppUrl(
                "Hello Inuka Afrika Properties, I would like help choosing a plot in Kilifi County."
              )}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 rounded-full bg-[#25D366] px-6 py-3 font-montserrat font-semibold text-white transition hover:bg-[#1ebe5d]"
            >
