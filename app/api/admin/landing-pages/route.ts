...
import { listLandingPages, saveLandingPage } from "@/lib/landing-pages/store";
...
export async function GET() {
  ...
  try {
    const pages = await listLandingPages(supabase);
    return NextResponse.json({ pages });
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not load landing pages" },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const page = await saveLandingPage(payload, undefined, supabase);
