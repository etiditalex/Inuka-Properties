  ...
  return `
    <div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto">
      <div style="background:linear-gradient(135deg,#0284c7,#075985);padding:24px;border-radius:12px 12px 0 0">
        <h1 style="color:white;margin:0;font-size:20px">IAPL Admin — ${payload.type === "inquiry" ? "New Inquiry" : "New Lead"}</h1>
      </div>
      <div style="background:#fff;padding:24px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 12px 12px">
        <table style="width:100%;border-collapse:collapse">${body}</table>
        <p style="margin-top:20px;font-size:13px;color:#6b7280">
          <a href="${process.env.NEXT_PUBLIC_SITE_URL || "https://www.inukaproperties.co.ke"}${adminPath(payload.type === "inquiry" ? "inquiries" : "leads")}" style="color:#0284c7">View in dashboard →</a>
        </p>
      </div>
    </div>
  `;
